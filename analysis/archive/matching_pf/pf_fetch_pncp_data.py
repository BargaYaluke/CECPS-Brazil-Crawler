# -*- coding: utf-8 -*-
"""纯契合版: 为 cat*_pf 标集补抓 PNCP 数据,与 v3 缓存合并 → *_pf.json。
   链接/ME-EPP 顺序慢速抓(限流);连通性异步并发(sem=5)。"""
import sys, io, json, re, time, asyncio
from collections import Counter
import httpx

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")
O = "analysis/matching/outputs"
...
pncps, links_in = [], {}
for f in ("cat1_pf.json", "cat2_pf.json", "cat3_pf.json"):
...
old_ref = json.load(open(f"{O}/pncp_links_refetch_v3.json", encoding="utf-8"))
old_me = json.load(open(f"{O}/meepp_benefit_v3.json", encoding="utf-8"))
old_conn = json.load(open(f"{O}/link_connectivity_v3.json", encoding="utf-8"))
...
print(f"目标 {len(pncps)} 标 | 链接补抓 {len(todo_ref)} | ME/EPP补抓 {len(todo_me)}")
...
with httpx.Client(timeout=45, headers=HEAD, follow_redirects=True) as cli:
    for i, pn in enumerate(todo_ref):
        ...
        time.sleep(0.5)
        if (i + 1) % 25 == 0:
            print(f"  [links] {i+1}/{len(todo_ref)}", flush=True)
    ...
    for i, pn in enumerate(todo_me):
        ...
        time.sleep(0.5)
        if (i + 1) % 25 == 0:
            print(f"  [meepp] {i+1}/{len(todo_me)}", flush=True)
...
async def conn_main():
    ...
    async with httpx.AsyncClient(timeout=18, headers=BHEAD, follow_redirects=True, verify=False) as cli:
        async def probe(url):
            if not url or not str(url).strip():
                return {"status": None, "final_url": None, "err": "无链接"}
            if not str(url).lower().startswith(("http://", "https://")):
                return {"status": None, "final_url": None, "err": "链接格式无效(非http,源数据残缺)"}
            last = None
            for attempt in range(2):
                try:
                    r = await cli.get(url)
                    return {"status": r.status_code, "final_url": str(r.url), "err": None}
                except Exception as e:
                    last = f"{type(e).__name__}: {str(e)[:60]}"
                    await asyncio.sleep(1 + attempt)
            return {"status": None, "final_url": None, "err": last}
        async def work(pn):
            rec = ref.get(pn, {})
            eff = rec.get("orig_link") or rec.get("linkSistemaOrigem")
            async with sem:
                o = await probe(eff)
                p = await probe(rec.get("portal"))
            cat, _ = classify(o["status"], o["err"], o.get("final_url"))
            conn[pn] = {"pncp": pn, "公司": rec.get("公司"), "eff_link": eff,
                        ...
                        "situacao": rec.get("situacao"), "origin_status": o["status"],
                        "origin_err": o["err"], "origin_class": cat,
                        "portal": rec.get("portal"), "portal_status": p["status"], "portal_err": p["err"]}
        await asyncio.gather(*[work(p) for p in todo_conn])

asyncio.run(conn_main())

json.dump(ref, open(f"{O}/pncp_links_refetch_pf.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
json.dump(me, open(f"{O}/meepp_benefit_pf.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
json.dump(conn, open(f"{O}/link_connectivity_pf.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
...
for k, n in Counter(v.get("label") for v in me.values()).most_common():
    print(f"  {n:3}  {k}")
print("PNCP situacao 分布:")
for k, n in Counter(v.get("situacao") for v in ref.values()).most_common(8):
    print(f"  {n:3}  {k}")
